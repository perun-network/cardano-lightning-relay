...
pub(crate) fn handle_ldk_events<'a>(
	...
	async move {
		match event {
			...
			Event::PaymentClaimed { payment_hash, purpose, amount_msat, .. } => {
				...
				// Check if this payment is a swap and trigger Cardano fulfillment
				if let (Some(db), Some(op)) = (&swap_db, &operator_agent) {
					let hash_hex = format!("{}", payment_hash);
					if db.get_by_payment_hash(&hash_hex).is_some() {
						let op = Arc::clone(op);
						let db = Arc::clone(db);
						tokio::spawn(async move {
							cardano_swap::fulfill_swap(op, db, hash_hex).await;
						});
					}
				}
			},
